...
interface DataJoinEventProps {
  asosiasiId: number;
}

const DataJoinEvent: React.FC<DataJoinEventProps> = () => {
  const [umkmAsosiasi, setUmkmAsosiasi] = useState<UmkmAsosiasi[]>([]);
  const [loading, setLoading] = useState(true);
  const [asosiasiId, setAsosiasiId] = useState<string>();
  ...
  const fetchAsosiasiId = useCallback(async (user_id: any) => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/asosiasi/${user_id}`
      );
      setAsosiasiId(response.data.id || "");
